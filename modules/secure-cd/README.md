...
* **Cloud Deploy Pipeline & Targets**: Creates a delivery pipeline and deployment targets (GKE, Anthos, or Cloud Run) based on your environment definitions, supporting progressive multi-phase canary rollouts.
* **Canary Deployment & Guardrails**: Enables phased traffic shifting (e.g. 10%, 50%, 100%) with automated deployment verification (`verify = true`) and runtime traffic control for GKE and Cloud Run targets.
...
| deploy\_branch\_clusters | A list of environment deployments, ordered by 'env\_number'. target\_type can be one of `gke`, `anthos_cluster`, or `run`. Supports optional canary rollout configurations (`canary_percentages`, `runtime_config`). See [clouddeploy\_target Terraform docs](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/clouddeploy_target) for more details | <pre>map(object({<br>    name                  = string<br>    cluster               = string<br>    anthos_membership     = string<br>    project_id            = string<br>    location              = string<br>    required_attestations = list(string)<br>    env_attestation       = string<br>    env_number            = number<br>    target_type           = string<br>    canary_percentages    = optional(list(number))<br>    runtime_config        = optional(object({...}))<br>  }))</pre> | `{}` | no |
